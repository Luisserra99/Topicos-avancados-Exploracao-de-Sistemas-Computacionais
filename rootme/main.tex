...
um painel de \emph{upload} sem qualquer validação de tipo/extensão de arquivo
...
\texttt{www-data}. A partir desse \emph{foothold}, a enumeração local revela um
...
binário SUID $\rightarrow$ \texttt{root}.

%============================= 2.4 RECONHECIMENTO ===============================
\section{Reconhecimento}

\subsection{Varredura de portas}
A varredura inicial identifica quais serviços estão expostos e suas versões,
...
\emph{wordlist} \texttt{common.txt}. O objetivo é descobrir pontos de entrada
...
upload}''. Testes de envio mostram que a aplicação \textbf{não valida a
extensão nem o tipo (MIME) do arquivo} e armazena o resultado em
...
(\emph{unrestricted file upload $\rightarrow$ RCE}).
...
de escuta. O arquivo foi renomeado para \texttt{.php5} a fim de contornar um
eventual filtro sobre a extensão \texttt{.php}, mantendo a execução como PHP.
...
\texttt{4000}), que executam com os privilégios do dono do arquivo. Se algum
interpretador ou binário perigoso estiver marcado como SUID de \texttt{root},
ele pode ser abusado para elevar privilégios.
...
o painel web em \texttt{/panel} permitia \emph{upload} irrestrito de arquivos:
...
indevidamente marcado como \emph{SUID root}; abusá-lo via GTFOBins
(\texttt{sh -p}) elevou o acesso a \texttt{root}. A falha central foi a
